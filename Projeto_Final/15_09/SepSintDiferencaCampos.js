...
let qntdSintomasNaoPreenchidos = Array(65).fill(0)
...
async function getSintomas(data){
    ...
    let camposNaoPreenchidos = 0
    for(let i=0;i<vetorcampos.length;i++){
        if(linha[vetorcampos[i]] == '9' || linha[vetorcampos[i]] == '') camposNaoPreenchidos++ 
    ...
    qntdSintomasNaoPreenchidos[camposNaoPreenchidos]++;
}
...
fs.createReadStream('BancoBruto2020.csv')//Data.csv
.pipe(csv({}))
...
.on('end', () => {   //Lógica aplicada quando chega no EOF
    ...
    for(let i=0;i<qntdSintomasNaoPreenchidos.length;i++){
        console.log("Tem %d pacientes com %d campos não preenchidos",qntdSintomasNaoPreenchidos[i],i)
    }
