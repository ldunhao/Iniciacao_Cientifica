...
function GeraArvore(BD,YlinhaBD,WP){ //BD[1] YlinhaBD[1]
  let Arvore = new No
  let Arr = []
  let countV = 0
  let countM = 0
  ...
  if(t < 0.95){//&& BancoDados.length > 1000
    ...
    Arvore.right = GeraArvore(BDd,YlinhaD,W)
    Arvore.left = GeraArvore(BDe,YlinhaE,W)
    //Arvore.data = W
    return Arvore
  }
  else{
...
async function GeradorBD(){
    BD = []
    Ylinha = []
    await fs.createReadStream('MiniBanco.csv')
    .pipe(csv({}))
